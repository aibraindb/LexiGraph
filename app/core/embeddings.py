...
class SimpleVectorStore:
    def __init__(self, dirpath: str):
        ...
        self._X = None; self._meta = []; self._nn = None; self._vectorizer = None
        self._load()
    ...
    def add(self, text: str, doc_id: str, fibo_class_uri: str|None=None):
        ...
        self._X = X
        self._meta.append({"doc_id": doc_id, "text": text, "fibo_class_uri": fibo_class_uri})
        with open(self.vectorizer_path,"wb") as f: pickle.dump(self._vectorizer,f)
        np.save(self.vecs_path, self._X.astype("float32"))
    ...
    def search(self, text: str, topk: int=8):
        if self._X is None or self._nn is None: return []
        ...
        for sim, idx in zip(sims, idxs[0]):
            ...
            m = self._meta[idx]
            out.append({"doc_id": m["doc_id"], "sim": float(sim), "fibo_class_uri": m.get("fibo_class_uri")})
        return out
